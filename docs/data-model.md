...
| `reportPostSlug`                               | string?          | Optional slug of a standalone `posts/{postId}` article used as the event report. `/events/[slug]` links to `/blog/{reportPostSlug}` only when that post is `published`; the report remains an article, not a synthetic event.                                                                                                                                               |
...
| `isGuest`                                   | boolean?                                        | Marker for attendee rows not backed by `users/{uid}`. Admins can create these for historical/offline attendees; current QR check-in sends logged-out visitors through Google login and returns them to the check-in URL. **Server-managed**, same as `attendedAt`. |
| `createdAt`, `updatedAt`                    | Timestamp                                       |                                                                                                                                                                                                           |
